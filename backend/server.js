...
app.post('/analyze', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file uploaded. Provide a PDF or TXT file.' });
    }
    ...
    if (text.length < 800) {
      return res.status(422).json({
        ...
        detail: `Extracted ${text.length} characters. Minimum 800 required for meaningful analysis.`,
      });
    }
